...
适配层"错家形状"预检 · 工具往返回填构造器 · 门面文档与图收敛（全部是兼容性新增）。
...
- **README 新增重点段落「它屏蔽了什么」**（放在最前，回答"为什么值得引入"）：六层屏蔽清单
  （传输与认证 / 请求形状 / 响应与归一 / 流式 / 可靠性 / 解码，每层附"为什么自己写容易错"）·
  **四个真实跑出来的例子**（同一条 `Message` 在两个协议下的请求体 · usage 十几种写法归一到 6 个键 ·
  错误体两种嵌套归一到同一类失败 · 分片边界：SSE 行被切开与汉字断在分片中间）·
  四种手段（翻译/归一/兜底/吸收）与"保留原始事实"原则 · 故意不屏蔽清单 · 一句判据 ·
  "不省什么 / 什么时候不该用它"。例子由新增的 `scripts/show_abstraction_examples.py` 复现
  （离线，纯函数 + 已有实现，README 贴的就是它的输出）。原「它省掉什么」段与其重叠，已合并删除。
...
- 归档 `tests/v04/.work/cli_smoke/`（旧 DX 时代的示例工程，import 的 `weave.dx.*` 早已不存在，
  因不叫 `test_*.py` 而从未被测试发现）→ `archive/v0.4-parked/dx-smoke/`；
  `.gitignore` 增加 `.work/`（测试的运行时目录不该入库），并清掉本地 241 个测试残留 `.db`。
- **版本 `0.4.0.dev0` → `0.5.0`**：本批全部是兼容性新增（`str` 路径零变化、只加方法与字段），
  无破坏性变更。
