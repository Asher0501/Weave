...
from weave_agent_sdk.loop.iterative import IterativeLoop, _format_memory
...
**修复建议**: 将导入改为 `from weave_agent_sdk.loop.base import format_memory_context as _format_memory`，或将第 48 行的调用改为 `format_memory_context(memory_ctx)`。
...
from weave_agent_sdk.types import Message  # ← 在方法体内部导入
...
**建议**: 将 `from weave_agent_sdk.types import Message` 移到文件顶部。
...
| `scheduled.py` 导入位置 | ⚠️ **P3 新增** | 方法体内 `from weave_agent_sdk.types import Message` |
...
| 🔴 **P0** | `_format_memory` 导入错误导致 `ScheduledLoop` 运行时崩溃 | `weave/loop/scheduled.py:40` | 改为 `from weave_agent_sdk.loop.base import format_memory_context as _format_memory` |
...
| 🟢 **P3** | `from weave_agent_sdk.types import Message` 在方法体内 | `weave/loop/scheduled.py:47` | 移至文件顶部 |
...
| 1 | `scheduled.py` `_format_memory` 导入错误 | 🔴 P0 | 改为 `from weave_agent_sdk.loop.base import format_memory_context` | `scheduled.py` 顶部已导入 `from weave_agent_sdk.loop.base import BaseLoop, format_memory_context`，`_execute_once()` 中调用 `format_memory_context(memory_ctx)` | ✅ **已修复** |
...
| 4 | `scheduled.py` 方法体内 `import` | 🟢 P3 | 移至文件顶部 | `from weave_agent_sdk.types import LoopResult, Message` 和 `from weave_agent_sdk.loop.base import ...` 均在文件顶部 | ✅ **已修复** |
...
| `scheduled.py` 导入 | ✅ **已修复（验证通过）** | 正确导入 `from weave_agent_sdk.loop.base import BaseLoop, format_memory_context`，无死代码 |
